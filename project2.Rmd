...
Now we train a model based of the same arcitecture except instead of using Relu as our activation function we instead opt to using the tanh activation function.
...
Batch normalization should be done right before implementation of an activation function in a model.
...
From the figure above we notice the test accuracy is higher in the model with batch normalization.
